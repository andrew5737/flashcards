...
describe("initializeApi", () => {
  it("dispatch request action", async () => {
    await initializeApi();
    expect(dispatchListener.mock.calls[0][0].type).toBe(getType(actions.initializeApi.request));
  });

  it("dispatch request action followed by success action", async () => {
    await initializeApi();
    expect(dispatchListener.mock.calls[0][0].type).toBe(getType(actions.initializeApi.request));
    expect(dispatchListener.mock.calls[1][0].type).toBe(getType(actions.initializeApi.success));
  });
  ...
  it("dispatches requestion action followed by failure action when getAllDecks is rejected", async () => {
    ...
    await initializeApi();
    expect(dispatchListener.mock.calls[0][0].type).toBe(getType(actions.initializeApi.request));
    expect(dispatchListener.mock.calls[1][0].type).toBe(getType(actions.initializeApi.failure));
  });
